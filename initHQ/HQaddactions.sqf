...
if (isServer) then {
    _hq addaction ["<t color='#00b7ff'>Rest (wait/save)</t>",{ [] call DUWSR_fnc_restServer }, "", 0, true, true, "", "_this == player"];
};

if (!isServer) then {
    _hq addaction ["<t color='#00b7ff'>Rest</t>",{ [] call DUWSR_fnc_restClient }, "", 0, true, true, "", "_this == player"];
};
